# GenAI Learning Repository

![Generative AI](https://raw.githubusercontent.com/microsoft/generative-ai-for-beginners/main/images/repo-thumbnails/home.png)

A comprehensive collection of learning materials and practical implementations for Generative AI and Large Language Models.

---

## Contents

### Core Modules

1. [Introduction to GenAI and LLMs](01.Introduction_to_GenAI_and_LLMs.md)
2. [AI Infrastructure](02.AI_Infrastructure.md)
3. [Large Language Models Fundamentals](03.Large_Language_Models_Fundamentals.md)
4. [Transformer Architecture](04.Transformer_Architecture.md)
5. [Attention Mechanisms](05.Attention_Mechanisms.md)
6. [Embeddings and Vector Databases](06.Embeddings_and_Vector_databases.md)
7. [Retrieval Augmented Generation (RAG)](07.Retrieval_Augmented_Generation_RAG.md)
8. [Prompt Engineering](08.Prompt_Engineering.md)
9. [Fine-Tuning and Training](09.Fine_Tuning_and_Training.md)
10. [Model Evaluation and Metrics](10.Model_Evaluation_and_Metrics.md)
11. [Deployment and Inference](11.Deployment_and_Inference.md)

### Practical Implementations

- **Ollama**: Local LLM setup and usage guide - [View Guide](Ollama/ollama.md)
- **Embeddings**: Token embeddings implementation - [View Notebook](Embedings/Token-Embeddings.ipynb)

---

## Topics Covered

📌 Fundamentals of Generative AI and LLMs
📌 Deep learning architecture (Transformers, Attention mechanisms)
📌 Vector databases and semantic search
📌 Retrieval Augmented Generation (RAG) patterns
📌 Prompt engineering techniques
📌 Model training and fine-tuning
📌 Evaluation metrics and benchmarks
📌 Production deployment strategies
📌 Local LLM deployment with Ollama


---

## Structure

Each module is self-contained with:
- Comprehensive explanations of concepts
- Code examples and implementations
- Visual diagrams and illustrations
- Key takeaways and best practices

---

## Prerequisites

- Basic understanding of Python programming
- Familiarity with machine learning concepts (helpful but not required)
- For practical notebooks: Jupyter environment and required Python packages

---

## Contributing

This repository is a living collection of GenAI learning materials. Feel free to suggest improvements or additions.

---

## Connect with Me

Connect with me on LinkedIn: [Rgahul M](https://www.linkedin.com/in/m-raghul/) ❤️